...
public class OrderDAO {
	...
	// [사용자]목록/검색글 목록
	public List<OrderVO> getListOrderByMem_num(int startRow, int endRow, String keyfield, String keyword, int mem_num)throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<OrderVO> list = null;
	...
	public void updateOrder(OrderVO order)throws Exception{
		...
		try {
			...
			pstmt.setString(8, order.getReceive_notice());
			...
			if(order.getStatus() == 5) {
				...
				for(int i=0; i<detailList.size(); i++) {
					...
					pstmt2.addBatch(); // sql문을 메모리에 올린다.
					
					// 계속 추가하면 outOfMemory 발생, 1000개 단위로 executeBatch()
					if(i % 1000 == 0) { // 메모리에 저장한 sql문이 1000개인 경우
						pstmt2.executeBatch(); // 메모리에 저장된 sql문 일괄 실행
					}
