...
public class UpdateReplyAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		}else if(user_num != null && user_num == db_reply.getMem_num()) { // 로그인 O (로그인된 회원과 댓글 작성 회원이 일치하는 경우)
		...
		}else { // 로그인 O (로그인된 회원과 댓글 작성 회원이 불일치 하는 경우)
			mapAjax.put("result", "wronngAccess");
		}
